...
class RegionAttention(nn.Module):
    def __init__(self, sorted_map, d_model=200, num_heads=8, dropout=0.1, batch_first=True):
        ...
        self.intra_group_attention = nn.MultiheadAttention(embed_dim=d_model, num_heads=num_heads, dropout=dropout, batch_first=batch_first)
    ...
    def forward(self, x):
        ...
        informed_complete_x = self.intra_group_attention(complete_x, complete_x, complete_x)[0]
        informed_complete_x = informed_complete_x.reshape(Bz * num_patch, num_chans, num_full_groups, group_size, patch_size)
        
        # Average over the `group_size` dimension (dim=3)
        group_mean = torch.mean(informed_complete_x, dim=3) 
