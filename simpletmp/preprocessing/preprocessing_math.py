...
root_dir = os.path.expanduser('~/simpletmp/data/MentalArithmetic/eeg-during-mental-arithmetic-tasks-1.0.0')
...
db = lmdb.open(os.path.expanduser('~/simpletmp/processed_data/math'), map_size=1000000000)
for files_key in files_dict.keys():
	for file in files_dict[files_key]:
		...
		if a != 0:
			eeg = eeg[:, :-a]
		eeg = eeg.reshape(19, -1, 5, 200).transpose(1, 0, 2, 3)
		label = int(file[-5])
