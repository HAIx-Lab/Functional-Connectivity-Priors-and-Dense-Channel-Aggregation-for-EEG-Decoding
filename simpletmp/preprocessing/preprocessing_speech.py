...
train_dir = os.path.expanduser('~/simpletmp/data/BCIC2020-3/Training set')
val_dir = os.path.expanduser('~/simpletmp/data/BCIC2020-3/Validation set')
test_dir = os.path.expanduser('~/simpletmp/data/BCIC2020-3/Test set')
...
db = lmdb.open(os.path.expanduser('~/simpletmp/processed_data/speech'), map_size=3000000000)
...
df = pd.read_excel(os.path.expanduser("~/simpletmp/data/BCIC2020-3/answer_sheet_track3.xlsx"))
df_=df.head(53)
all_labels = df_.values
print(all_labels.shape)

# FIX 1: Changed 1:30:2 to 1::2 to capture all alternating columns dynamically
all_labels = all_labels[2:, 1:][:, 1::2].transpose(1, 0) 

print(all_labels.shape)
print(all_labels)

for j, file in enumerate(files_dict['test']):
    # FIX 2: Safely break the loop if test files outnumber the available labels
    if j >= len(all_labels):
        print(f"Stopping early: No more labels available for file {file}")
        break
